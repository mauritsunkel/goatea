...
plot_genelists_overlap_venn <- function(genelists) {
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_polygon(ggplot2::aes(.data$X, .data$Y, fill = .data$id, group = .data$id), data = ggVennDiagram::venn_regionedge(d), show.legend = F) +
    ggplot2::geom_path(ggplot2::aes(.data$X, .data$Y, color = .data$id, group = .data$id), data = ggVennDiagram::venn_setedge(d), show.legend = F) +
...
## DEPRECATED: UpsetR plot version
